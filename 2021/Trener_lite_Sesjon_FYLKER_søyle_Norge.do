...
*
/* 	Spec: 
	Søyler for hvert fylke, pluss landet. Versjon med egen søyle for landet.
...
* på horisontal akse, og y-options på vertikal!
* "twoway bar ..., horizontal"   legger søylene flatt, men y-aksen er fremdeles "Andel" og går til 75.
* "Hbar" er ikke noen twoway-graf.

* Mekke til rekkefølgen av fylkene - VELG METODE:

/* A) I GEOGRAFISK REKKEFØLGE
...
*	else if "`geonivaa'"=="fylke" {
	graph twoway ///
		...
		///yline(`normal_linje') 									///
		xlabel(1/$antall, noticks valuelabel angle(45) labsize(*`tekstskalering')) xscale(titlegap(3) range(`xmin' `xmax') )	///
		xtitle("")			///
		graphregion(fcolor(white) lcolor(white) ilcolor(white)) ///
		...
		(bar fylkestall radnr if geo == 0 ,			///		Søyle for landstallet
