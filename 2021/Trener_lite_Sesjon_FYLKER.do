...
*
/* 	Spec: 
	Søyler for hvert fylke, pluss landet. Prøver med landet som linje (som det er i mal-figuren).
...
* Ta vare på landstallet: HER ER BARE ÉN VERDI, det er ikke Ungdata.
local landstall = verdi_referansenivaa
* Ta vare på fylkestallene
keep sted_kode indikator verdi_mellomgeonivaa datotag_side4_innfilutfil
...
*	if "`geonivaa'" == "kommune" drop if strlen(Sted_kode)>4
	...
	****Flytte fylkestallet inn på raden for fylket
	...
	generate fylkestall = real(verdi_[_n+1])
...
quietly describe
global antall=r(N)
* Mekke til rekkefølgen av fylkene
gen radnr = .	//løkkestyring
replace radnr = 1 if GEO == 30
replace radnr = 2 if GEO == 3
replace radnr = 3 if GEO == 34
replace radnr = 4 if GEO == 38
replace radnr = 5 if GEO == 42
replace radnr = 6 if GEO == 11
replace radnr = 7 if GEO == 46
replace radnr = 8 if GEO == 15
replace radnr = 9 if GEO == 50
replace radnr = 10 if GEO == 18
replace radnr = 11 if GEO == 54
...
*Lage en label til x-aksen i fylkesgrafene: Må koble navnene til andre tall 1-2-3...
*forvalues i = 1/4 {
sort radnr
...
*	else if "`geonivaa'"=="fylke" {
	graph twoway ///
		(bar fylkestall radnr ,		/// 
		color("`kommunefarge'") barwidth(0.8) 						///
		ylabel(0(10)`ymax', angle(horizontal) valuelabel glcolor(white)) ///		
		ytitle("`yaksetekst'", size(medium) orientation(vertical)) ///
		yscale(range(`ymax')) ///  Uten denne får ikke øverste ylabel noen gridline.
		///yline(`normal_linje') 									///
		xlabel(1/11, noticks valuelabel angle(45) labsize(*`tekstskalering')) xscale(titlegap(3) range(0.5 11.5) )	///
		xtitle("")			///
		graphregion(fcolor(white) lcolor(white) ilcolor(white)) ///
		...
		(bar fylkestall radnr if radnr == `i',		///		Uthever aktuelt fylke 
