...
# Reshape til én verdikolonne, og så plotte med 'group' for å skille byene.
# Siden det kunne være ulike landstall for ulike byer (år!) bruker jeg fylkeskoden som Geo på landstallet.
# Lage en sorteringsvariabel (x-akse).
...
plotdata <- plotdata %>%                                      # Legge inn kommune- og FYLKESkode for K- og L-tall
  ...
  mutate(sortering = group_by(., hjemmeby)) 


plotdata %>%
  split(.$hjemmeby) %>%
  map(~ggplot(.) +
        geom_col(mapping = aes(x = sortering, y = verdi), stat = "identity") +
        labs(x = "Bydeler", y = "Andel lite aktive (prosent)" ) +
        annotate(geom = "text", x = Inf, y = Inf, label = "Indikator.txt batchnr=")
 )
plotdata
...
# DIVERSE SOM IKKE FUNKET: Etter Pivot(), replace Sted_kode med hhv. 4- og 2-sifret kode for geonivåene.
    # plotdata %>% 
