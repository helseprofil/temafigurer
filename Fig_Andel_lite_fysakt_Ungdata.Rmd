...
```{r Verifisere-indikator-med-Assertr, eval=TRUE}
# Fant package assertr, med god veiledning.

# Jeg må ha en "string starter med"-funksjon! Det er jo årstall bak.
# Denne virker, separat: a får celler med FALSE der teksten stemmer (og TRUE resten).
# a <- str_starts(datasett$Indikator, "Lite fysisk aktive", negate = TRUE)
...
sjekktekst <- function(x) str_starts(x, indikatortittel, negate = FALSE)

sjekk <- datasett %>% 
  select(LPnr, Indikator) %>%
  filter(LPnr == indik_nr) %>%
  #assert(in_set("Lite fysisk aktive"), Indikator)
...
```{r Plotte_kommune-og-Land-også}
plotdata <- datasett %>%
  filter(LPnr == indik_nr, SpraakId == "BOKMAAL") %>%
  select(Aar, Sted_kode, Sted, LPnr, Indikator, contains("Verdi_"), Datotag_side4_innfilutfil, Enhet) %>%
  mutate(hjemmeby = str_sub(.$Sted_kode, start = 1, end = 2)) 
# %>%
#   split(.$hjemmeby) %>%
#   map(~ggplot(.) +
#         geom_col(mapping = aes(x = Sted_kode, y = Verdi_lavesteGeonivaa)) +
#         geom_col(mapping = aes(x = Sted_kode, y = Verdi_mellomGeonivaa)) +      # NEI, blir jo bare plottet oppå!
#         labs(x = "Bydeler", y = "Andel lite aktive (prosent)" ) +
#         annotate(geom = "text", x = Inf, y = Inf, label = "Indikator.txt batchnr=")
#  )
plotdata
