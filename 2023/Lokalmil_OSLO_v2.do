* TEMAFIGUR 2023: FORNØYD MED LOKALMILJØET (Ungdata) - EGEN GRAF FOR OSLO "FYLKE"
* 
...
Endringer/utvikling: 
	...
	- 15.03.2023: Skilt ut i eget script: Laget en egen Oslo-graf med bydelstall ist.f. kommuner.
	  Leser inn Indikator.txt for BYDELER.
...
else if "`geonivaa'" == "fylke" { //For OSLO: Bruker datafil for BYDELER.
	local lokmilj = 18
} 
*INNDATA:
* For UTVIKLING
...
* SKARP - Tar den som faktisk er lastet opp FOR BYDELER - fra backup av batchen.
local datakatalog "F:\Forskningsprosjekter\PDB 2455 - Helseprofiler og til_\Profiler\0_Sikkerhetskopier\2023_PDF_FRYS\Folkehelseprofiler\bydel\Bydel-2023-02-08-16-28"
...
* Ta vare på landstallet: HER SKAL VÆRE BARE ÉN VERDI for alle tre år!
	...
	//- Til søyle for landet inniblant : legge landstallet i BYDELStall-kolonnen.
	//  Det må trikses til lengre ned.
local landstall = verdi_referansenivaa
local oslotall  = verdi_mellomgeonivaa
...
* Må beholde landet, Oslo og bydeler i Oslo. 
drop if geo > 3 & geo < 30000
drop if geo > 100000 
drop if geo > 30115		//Sentrum og Marka
drop _merge
...
*	if "`geonivaa'" == "kommune" drop if strlen(Sted_kode)>4
	...
	****Lage landstall som kan plottes inni hvert fylke
	levelsof hjemmefylke if geo > 3 , local(f_liste)	//Telle fylker
	...
	replace verdi_lave = "`oslotall'" if geo == 3		//Fyller inn verdien, til plottet
...
/*	forvalues f = 1/`ant_fylker' {						//Tilordne fylkesangivelse til landstall-radene
	...
	replace hjemmefylke = 301 							//Gjelder alle som ikke allerede er det!
	replace geo = 1000 if geo == 0						//For å få landstallet sortert som en kommune
	replace geo = 1100 if geo == 3
...
* C) Viser ALLE GEO etter synkende indikatorverdi.
*    Må resette radnr, som brukes til labelling.
*    _Antallet_ radnr per fylke er uendret, så forsterad og sisterad stemmer.
*replace verdi_lave = "99999" if length(Sted_kode) == 2 & geo != 1000	//OBS unngå landstallet, 'geo 1000'.
gsort hjemmefylke -verdi_lave geo		//Inkl. geo for å stabilisere sorteringen ved ties!
replace radnr = _n	
...
* OSLO-figur: Bruker den mellomblå kommunefargen som LANDSFARGE,
* og en lys grå for bydeler som KOMMUNEFARGE.
* Kodene fra paletten skal være blå #2ea1c0 (rgb 46 161 192), grå #cccccc (rgb 204 204 204).
* I Stata kreves andre koder for å få riktig farge (!)
local kommunefarge "204 204 204*1.07"	//#cccccc, "grey80" i R. Til bydelene.
local landsfarge "39 39 114*1.6"		//Mørk blå, lik banneret i FHP Kommune 2022 (tilpasset selv!) Til landet.
local oslofarge "35 149 186"			//#2ea1c0 . mellomblå, lik kommunefargen i de andre fylkesgrafene.

* Unicode-tegn for landslinja, til forklaringstekst  - IKKE I BRUK.
...
local i=8 //Oslo har rad 8 i 2023-grafen
...
****Aksetekster
	...
	*if "`geonivaa'" == "bydel" local xaksetekst "`fylkenavn'" 	//For bydel er "fylkenavn" hardkodet tekst.
	local xaksetekst "Bydelene i `fylkenavn'" 				//Oslo har ingen underliggende kommuner
*di "Xaksetekst, som inneholder fylkenavn: `xaksetekst' "

	//Dimensjonere plassering av tekster og andre ting
	local minste_x = forsterad[`i'] 
...
*	else if "`geonivaa'"=="fylke" {
	graph twoway ///
		...
		ylabel(0(10)$ymax, angle(horizontal) valuelabel glcolor(gs14))	///		
		...
		caption("Lokalmiljø: Indikator.txt for Bydeler, batchnr. `batchnr'", position(5) ring(5) size(vsmall) color(gs9) )	///
		...
		(bar andel radnr if hjemmefylke==$hjemfylke & geo == 1100 ,		/// Oslotallet som egen søyle
		color("`oslofarge'") barwidth(0.8) 							///
		)
