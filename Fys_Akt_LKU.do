* TEMAFIGUR 2021: FYSISK AKTIVITET FRA LKU (Fylker)
* 
*		OBS VED GJENBRUK: Ny mappestruktur for produktene ifm OVP sep-2020.
*
/* 	Utgangspunkt: Trener_lite_Sesjon
...
C:\...\stbj\Desktop\Git_lokale_repo\Temafigurscript
...
local geonivaa ="fylke" //Tillatte verdier: "fylke" . Annen figur i K og B.
...
*INNDATA:
local kildekatalog "F:/Forskningsprosjekter/PDB 2455 - Helseprofiler og til_/PRODUKSJON\PRODUKTER\KUBER\NORGESHELSA\NH2021NESSTAR"
...
local inndata "FYSAK_LKU_2021-03-01-14-30"
...
*else local targetkatalog "F:/Forskningsprosjekter/PDB 2455 - Helseprofiler og til_/PRODUKSJON\PRODUKTER\SSRS_filer/FHP/`profilaar'/`geonivaa'\Temafigurer\Fysakt_LKU"
...
local ymax = ceil(`ymax') 		//Runder oppover til nærmeste heltall
	...
	*Overstyring: Det er vanlig at folket vil ha en spesifikk akselengde.
	local ymax=75		
	di "ymax: " `ymax'
...
*Overstyre: Som regel skal aksen starte på null.
local ymin =0
	di "ymin: " `ymin'
*pause
*-------------------------------------------------------------------------------
...
if "`modus'" == "TEST" {
	local ifsetning = "i = 7/7"	//Lager én graf
}
